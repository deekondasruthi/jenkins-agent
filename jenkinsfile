            ...
            steps {
                script {
                    if (env.BRANCH_NAME == 'sub-1') {
                        echo "📂 Printing dockerfile content from sub-1"
                        sh '''
                            echo "---------------- dockerfile ----------------"
                            cat dockerfile || echo "❌ dockerfile not found in sub-1 branch"
                            echo "-------------------------------------------"
                        '''
                    } else if (env.BRANCH_NAME == 'sub-2') {
                        echo "📂 Printing sample.yaml content from sub-2"
                        sh '''
                            echo "---------------- sample.yaml ----------------"
                            cat sample.yaml || echo "❌ sample.yaml not found in sub-2 branch"
                            echo "---------------------------------------------"
                        '''
                    } else {
